...
class HomeFragment : Fragment() {
    ...
    private fun displayUsageStats() {
        ...
        val userInstalledApps = activity?.packageManager?.queryIntentActivities(
            Intent(
                Intent.ACTION_MAIN,
                null
        ...
        if (stats != null) {
            ...
            val filteredStats =
                sortedStats.filter { userInstalledApps?.any { app -> app.activityInfo.packageName == it.packageName } == true }
    ...
    private fun hasUsageStatsPermission(): Boolean {
        val appOps =
            activity?.getSystemService(Context.APP_OPS_SERVICE) as android.app.AppOpsManager
    ...
    override fun onCreateView(
        ...
        if (!hasUsageStatsPermission()) {
            ...
            activity?.registerReceiver(dataChangeReceiver, filter, Context.RECEIVER_NOT_EXPORTED)
