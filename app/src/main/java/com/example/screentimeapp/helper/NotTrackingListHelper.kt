...
import com.example.screentimeapp.model.NotTrackingRecord
import com.example.screentimeapp.model.UsageSummary
...
object NotTrackingListHelper {
    ...
    fun getList(context: Context): List<NotTrackingRecord> {
        ...
        val summary = UsageSummary.getSummary(context,
            ((time - 2 * UsageStatsHelper.HOUR_24)..time step UsageStatsHelper.HOUR_24).map { CalendarHelper.getDateCon(it) }
        )
